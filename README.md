...
사용자 신고로 드러난 결함이다. 근거가 없는 문의에서 "📮 접수 답변 만들기" 와
"🧭 조사 계획" 이 둘 다 실패했는데, 화면에는 **"큐에 추가하지 못했습니다"** 라는
무의미한 문구만 나왔다. 실제 원인은 **세션 만료(401)** 였다 — 다시 로그인하면 되는
상황인데, 사용자는 기능이 망가진 줄 알고 원인을 찾을 방법이 없었다.

원인은 둘이었고 둘 다 고쳤다.

1. **호출부가 사유를 버렸다.** `d.reason || d.error` 만 읽으니 서버가 401
   `{"detail": "..."}` 을 주면 아무것도 남지 않았다. `web/src/api.ts` 의
   `postJson/getJson` 이 상태 코드와 `detail` 을 문구로 바꾼다(401 → "세션이
   만료되었습니다", 403 → 권한, 5xx → 서버 오류). 401 은 전역 이벤트로 올려
   **로그인 화면으로 되돌린다** — 버튼마다 제각기 오류를 띄우면 사용자는 "이 기능이
   고장났다" 로 읽는다.
2. **세션이 재기동마다 끊겼다.** `RVP_SESSION_SECRET` 이 없으면 기동할 때마다 새
   서명 키가 생겨 **모든 로그인이 무효가 된다**. README 가 경고하고 있었지만 `.env`
   에는 없었다. 넣고 재기동 전후로 세션이 유지되는 것을 확인했다.

#### 근거가 없는 문의는 이렇게 처리한다

coverage 게이트에 막혔다는 것은 "이 유형은 KB 에 없다" 는 뜻이다. 그때 할 일은 넷이다.

| 단계 | 무엇 | 왜 |
|---|---|---|
| 1 | **📮 접수 답변** 발송 | 원인을 단정하지 않는 골격. 무응답이 가장 나쁜 실패다 |
| 2 | **지식 공백 기록** (`reply_no_evidence`) | 어떤 유형의 사례가 부족한지 집계된다(현황 → 지식 공백) |
| 3 | **🧭 1차 원리 조사 계획** | 근본원인을 지어내지 않고 "무엇을 재면 어떤 가설이 죽는지" 를 낸다 |
| 4 | 사람이 해결 → **분석 코멘트 게시** | KB 로 환류된다. 반복 문의 유형으로 묶으면 그 답변이 정본이 되어, 다음 같은 문의는 근거가 생긴다 |

이 경로가 실제로 도는지 확인했다(VOC-79, 근거 0건): 접수 답변 큐 적재 ·
`reply_no_evidence` 1건 기록 · 조사 계획 생성. 공백은 `/knowledge/gaps` 의
`top_underserved_templates` 로 올라와 무엇부터 보강할지를 가리킨다.

### 답변 지침 — Confluence · FAQ · 사내 문서

**사례와 지침은 역할이 다르다.** 사례는 "예전에 이랬다" 는 근거이고, 지침은 "이렇게
답해야 한다" 는 규칙이다. 그래서 프롬프트에서도 자리가 다르고, 충돌하면 **지침이
이긴다** — 규정이 바뀌었는데 옛 사례대로 답하면 틀린 답이 아니라 사고다.

`src/guides.py` 가 원천 세 종류를 같은 모양으로 접는다.

| 원천 | 형식 | 인증 |
|---|---|---|
| Confluence 페이지 | `confluence:<pageId>` | Jira 와 같은 자격증명(Cloud는 `/wiki`) |
| Confluence 스페이스 | `confluence-space:<KEY>` | 〃 (상한 100페이지 — 위키 전체를 빨아오지 않는다) |
| FAQ·문서 URL | `https://...` | `RVP_GUIDE_HEADERS="X-Api-Key: ..."` |
| 저장소 문서 | `file:data/knowledge.md` | — |

```sh
RVP_GUIDE_SOURCES=confluence:123456,https://wiki.example.com/faq,file:data/knowledge.md
curl -X POST localhost:8011/guides/sync     # 수집(관리자)
curl localhost:8011/guides                  # 현황 — 문서·섹션·실패 원천
curl -G localhost:8011/guides/search --data-urlencode 'q=권한 신청'
.venv/bin/python tests/test_guides.py       # 34개
```

#### 지침을 넣는 세 가지 방법

1. **화면에서 직접 작성** — 상단바 **📘 지침** → "직접 작성". 위키가 없어도 규칙을
   바로 넣을 수 있고, 저장 즉시 다음 답변부터 적용된다. 사람이 쓴 글은 수집본과
   **파일을 나눠** 저장한다(`data/guides_manual.json`) — 같은 파일에 두면 다음 수집이
   사람의 글을 덮어쓴다. 검색에서도 직접 작성한 것이 **앞에 온다**(동점이면 방금 쓴
   규칙이 이겨야 한다).
2. **화면에서 원천 연결** — 같은 화면의 "Confluence · FAQ · 문서 연결" 에 한 줄에
   하나씩 넣고 "저장하고 지금 수집". `.env` 를 고치고 재기동할 필요가 없다 —
   Jira·LLM 설정과 같은 저장값 경로(`app_config`)를 쓴다.
3. **환경변수** — 배포 자동화용. `RVP_GUIDE_SOURCES=confluence:123456,https://...`

화면에는 **"어떤 지침이 걸리나"** 미리보기가 있다. 실제 문의 문장을 넣어 무엇이
걸리는지 먼저 확인할 수 있다 — 여기서 안 걸리면 답변에도 반영되지 않는다.

설계에서 지킨 것:

- **문서를 통째로 넣지 않는다.** 제목(h1~h6 / `#`)에서 잘라 섹션 단위로 저장한다.
  긴 FAQ 한 장이 프롬프트를 다 먹고, 어느 문단이 근거인지도 말할 수 없게 된다.
  Confluence 는 섹션 앵커까지 붙여 그 문단으로 바로 보낸다.
- **파서는 내용으로 고른다.** `file:` 원천을 전부 마크다운으로 읽었더니 HTML 문서가
  통째로 한 섹션이 되고 본문에 태그가 남았다 — 확장자와 첫 바이트로 판별한다.
- **한국어 어미를 떼고 찾는다.** "환불해 주세요" 가 "환불은 …" 로 시작하는 지침과 한
  글자도 겹치지 않아 **규정이 있는데도 못 찾았다.** 어간 추출기는 `voc_agents` 가
  단일 소스다 — 두 벌이면 한쪽에서만 '환불' 이 되는 상태가 생긴다.
- **문서가 적을 때를 대비한다.** BM25 는 어떤 말이 문서의 절반에 나오면 IDF 가 0 이라
  **전부 0점**을 준다. 지침이 두어 개뿐인 도입 초기가 정확히 그 상황이라, 양수 점수가
  없으면 어휘 겹침으로 떨어지고 **제목 일치를 본문보다 2배로** 센다(안 그러면 "권한" 이
  스친 환불 섹션과 제목이 '권한 신청 절차' 인 섹션이 동점이 되어 규정을 잘못 인용한다).
- **무관한 질의에는 아무 지침도 주지 않는다.** 아무거나 끌어다 붙이면 규칙이 아니라 소음이다.
- **원천 하나가 실패해도 나머지는 수집한다.** 다만 조용히 넘어가지 않는다 — 실패는
  `/guides` 와 대시보드 카드에 남는다. 위키 한 장이 권한 때문에 막혔다고 지침 전체가
  비면, 답변은 지침 없이 나가면서 그 사실을 아무도 모른다.

적용된 지침은 답변 화면과 챗봇에 **📘 칩**으로 뜨고 누르면 원문 문단으로 간다 —
검토자가 "왜 이렇게 답했나" 를 물었을 때 답할 수 있어야 한다.

실측(사내 권한 요청 IVOC-44): 지침이 답변을 지배했다 — 승인 주체(팀장)·신청 경로(포털)·
처리 기간(영업일 1일)·"담당자가 직접 권한을 부여하지 않는다"·보안 준수의 "별도 채널로
전달" 이 모두 반영됐고, 과거 사례(LSI-49)는 근거로 인용됐다.

### 품질 판정 — 점수와 규칙을 구분한다

두 가지가 섞여 있었다. 분리했다.

| | `/rca/validate` (분석 코멘트) | `/voc/reply/review` (고객 답변) |
|---|---|---|
| 대상 | 엔지니어가 읽는 RCA 분석 | **고객에게 나갈 글** |
| 기준 | 근거 충실도·인용 정합·실행가능성·한자 금지 | 요청 응답 · 지침 준수 · 발송 적합 |
| 형태 | 1~10 점수 + 7점 통과 | **축별 예/아니오 + 이유** |
| 게이트 | 아님 | 아님 (막는 것은 정책 검사) |

**"품질 점수 10/10 (통과)" 는 과한 표현이었다.** 세 가지 문제가 있다.

1. **대상이 엇갈렸다.** 그 점수는 엔지니어용 분석을 잰다. 이 서비스의 최종 산출물은
   고객 답변인데 그쪽에는 판정이 없었다 — 재는 곳과 중요한 곳이 달랐다.
2. **자기 채점이다.** 기본값이 생성과 **같은 모델**이다(`RVP_JUDGE_MODEL` 미설정 시).
   이 저장소의 검증 하네스에서 그 한계가 실증됐다 — 30건 중 5건은 구조화 출력이
   깨졌고, "다른 고객사 정보는 공유할 수 없습니다" 라고 **올바르게 거절한** 답변을
   발송 불가로 매겼다. 그런 값에 "통과" 라는 말을 붙이면 사람이 검토를 건너뛴다.
3. **한 축으로 접었다.** 이 저장소는 초안 실패를 **레버별 원인**으로 분류한다 —
   무엇을 고쳐야 하는지가 나와야 하기 때문이다. 점수 하나는 그 반대 방향이다.

그래서 고객 답변 검토는 점수를 매기지 않는다. `요청에 답함 / 지침 준수 / 발송 적합`
각각에 예·아니오와 한 문장 이유만 낸다. 판정 형식이 깨지면 **판정 없음**이다 —
못 한 판정을 통과로 세지 않는다. 화면은 그 값이 **같은 모델의 의견**이라는 것과
발송을 막지 않는다는 것을 함께 보여준다.

**막는 것은 여전히 결정적 정책 검사다**(`voc_agents.check`). 규칙은 재현되고 이유를
말할 수 있지만, 모델 점수는 같은 입력에도 흔들린다. 흔들리는 값으로 발송을 막으면
막힌 이유를 설명할 수 없다.

### Jira 에 올라가는 형식 — 마크다운이 아니다

큐·미리보기·편집은 전부 **마크다운**이 정본이지만, 게시 직전에 `_md_to_jira()` 가
**Jira wiki markup** 으로 바꾼다(api/2 가 wiki 를 렌더하므로).
...
**번호 목록을 그대로 두면 목록이 아니라 평문으로 렌더된다** — 답변의 "권장 조치 순서"
가 통째로 한 문단으로 뭉개진다. 실제로 그렇게 나가고 있었고, 이번에 고쳤다(문장 중간의
"1. " 은 건드리지 않는다). 굵게 표시를 평문화하는 이유는 따로 있다 — Jira 볼드 `*x*`
는 닫는 `*` 에 한글 조사가 붙으면(`*CRC*를`) 렌더가 깨져 별표가 그대로 노출된다.

### 이슈 질문(챗봇) — 자연어로 묻고 이슈 내용으로 답한다

상단바 **💬 이슈 질문**. Jira 이슈의 내용을 자연어로 묻는다. 검색은 recommender 가,
생성은 기존 스트리밍 경로가 한다 — 새로 만든 것은 **근거 조립과 인용 검증**이다
(`src/issue_chat.py`).

세 가지를 지킨다.

1. **근거 밖의 말을 하지 않는다.** 답변은 제공된 이슈 발췌에서만 나오고, 없으면
   "확인되지 않습니다" 라고 답한다. 담당자가 이 답을 믿고 고객 답변을 쓰므로,
   그럴듯한 추측은 틀린 답보다 나쁘다 — 틀린 줄도 모르고 전달된다.
2. **이슈 키를 인용하되, 지어낸 키는 잡아낸다.** 고객 답변과 정반대 규칙이다
   (읽는 사람이 사내 담당자다). 근거에 없는 키가 본문에 나오면 완료 이벤트가
   `unsupported_mentions` 로 표시하고 화면이 빨간 줄로 경고한다 — 조용히 두면
   담당자가 그 번호를 찾으러 갔다가 없다는 것을 알기까지 시간을 쓴다.
3. **셀 수 있는 것은 세어서 준다.** "미해결 몇 건이야" 는 top-k 발췌로 절대 못 맞춘다
   (모델이 발췌 개수를 세거나 지어낸다). 집계는 검색이 아니라 **전수 계산**해서
   컨텍스트에 넣는다. 검색으로 못 푸는 질문을 검색으로 푸는 척하지 않는다.

실제로 이 원칙이 결함을 하나 드러냈다 — "미해결 중 어떤 분류가 많아?" 에 모델이
정직하게 "교차 집계가 제공되지 않았다" 고 답했다. 지어내지 않은 건 맞지만 답을 못 준
것이므로, 집계에 **미해결만/해결만** 분포를 추가했다.

- `POST /chat` 한 번에 받기 · `POST /chat/stream` SSE
  (`sources` → `delta`* → `done{citations, unsupported_mentions}`)
- 근거를 **먼저** 보낸다 — 답이 나오기 전에 무엇을 보고 답하는지 알아야 엉뚱한 근거를
  즉시 알아챈다.
- 후속 질문("그건 왜 그래?")은 직전 **사용자 질문**을 붙여 검색한다. 답변까지 붙이면
  답변 어휘가 검색을 지배해 엉뚱한 이슈로 흘러간다.
- `key` 를 주면 그 이슈로 범위를 좁힌다(`#/chat/LSI-7`).

```sh
curl -X POST localhost:8011/chat -H 'Content-Type: application/json' \
  -d '{"question":"무선 충전 중 태그 인식 실패는 어떻게 해결했어?"}'
.venv/bin/python tests/test_issue_chat.py     # 30개
```

**이슈 번호를 누르면 Jira 원문이 열린다.** 사용자가 이슈 번호를 클릭할 때 기대하는
것은 원문이다 — 예전에는 키가 **앱 안 이동**에 걸려 있었고 Jira 링크는 작은 `↗`
아이콘, 그마저 목록에서는 `opacity-0` 이라 hover 전에는 보이지도 않았다("Jira 링크를
연결할 수 없냐" 는 신고의 원인). 이제 키 자체가 링크이고, 앱 안에서 열기는 옆의
"여기서 열기" 버튼으로 남겼다. 답변·분석 코멘트·고객 답변 본문의
`LSI-7` 같은 키를 Jira `browse` 링크로 바꾼다(`web/src/issueLinks.tsx`). 코드 블록과
인라인 코드는 건드리지 않는다 — 로그 발췌 안의 문자열을 링크로 만들면 그 줄을 복사해
붙일 때 마크다운이 섞여 들어간다. base_url 은 `/config/status` 가 정본이라 배포마다
링크가 깨지지 않는다.

인용 형식이 `(LSI-7)` 인데 **여는 괄호 앞의 키가 링크되지 않는 결함**이 있었다 —
링크가 가장 필요한 자리가 통째로 빠져 있었다. 게다가 테스트가 정규식을 복사해 두고
있어 소스만 고쳤을 때 옛 규칙으로 초록을 냈다. 지금은 테스트가 **소스에서 정규식을
읽어 온다**(`tests/web/test_issue_links.mjs`, 9개).

> 화면 문구도 정리했다 — **"RCA 승인" → "분석 코멘트"**. 이 화면이 실제로 하는 일은
> 이슈에 남길 **내부 분석 기록**을 검토하고 게시하는 것이다. API 경로(`/rca/*`)와 코드
> 식별자는 그대로 뒀다 — 이름을 바꾸느라 배포·문서·이력이 갈라지는 비용이 표현 개선보다 크다.
...
**이 저장소는 같은 실수를 두 번 했다** — `guides` 와 `metrics` 가 빠져 지침 화면이
`Cannot read properties of null (reading 'sources')` 로 터지고 대시보드의 서빙 지연
카드가 조용히 비었다. 원인(프록시 설정)과 증상(널 역참조)이 멀어서 찾기 어렵다.
이제 둘로 막는다:
- `tests/web/test_api_prefixes.mjs` 가 **서버 라우트와 목록을 대조**한다(경로를
  추가하면 이 테스트가 먼저 실패한다). 사람이 기억하는 것에 맡기지 않는다.
- `web/src/api.ts` 가 200 인데 본문이 JSON 이 아니면 **그렇게 말한다** — 조용히
  null 을 돌려주지 않는다.
...
#### 사전 탐지 — 사람이 읽기 **전에** 잡는다 (`src/draft_defects.py`)

위 구조에는 구멍이 하나 있었다: 원인을 아는 시점이 **사람이 이미 초안을 읽고 고친
뒤**다. 검토 한 번이 통째로 낭비되고, 같은 결함은 프롬프트 규칙이 활성화될 때까지
(같은 클래스에서 2회 반복) 계속 나간다.

그런데 분류 체계를 만들고 보니 상당수는 초안과 근거 목록만 있으면 **즉시 확인
가능**했다. `draft_defects.detect()` 가 그것들을 초안 생성 시점에 잡아 검토 화면에
띄운다(같은 원인 코드·같은 레버 — 어휘가 하나여야 사전 지적이 사후 통계로 이어진다).

**오탐 0 을 정탐보다 우선한다.** 게이트로 쓰이는 신호가 멀쩡한 초안을 잡으면 사람이
곧 무시하고, 그러면 탐지기는 없는 것과 같다. 그래서 두 등급으로 나눈다 —
`certain`(대조할 정답이 있음: 근거 키 목록·섹션 규격·한자)만 차단 후보이고,
`likely`(문면 추정: 단정 표현·일반론·요지 이탈)는 **절대 막지 않고 경고만** 한다.
막는 근거가 재현되지 않는 게이트는 곧 꺼진다.

초안 **종류마다 약속이 다르다**는 것도 여기서 배웠다. LLM 심층 분석 기준(불확실성
섹션·번호 절차)을 제안 기반 초안에 들이대니 운영 KB 의 멀쩡한 초안이 **전부** 결함으로
잡혔다. 없는 약속을 어겼다고 할 수는 없다 → `DRAFT_KINDS` 로 계약을 분리했다.
마찬가지로 Jira 위키(`h3.`) 형식 본문을 못 읽어 "섹션 없음" 으로 단정한 것도 고쳤다 —
**못 읽는 것과 없는 것은 다르다.**

실측(운영 KB 40건 초안): 지적 3건. 그중 2건이 근거 목록에 없는 키 인용이었고,
추적해 보니 큐레이션 레코드 `LSI-120-rca` 의 `workaround` 필드에 봇 댓글 꼬리
(`참고 사례: … LSI-205`)가 통째로 들어가 있어 새 초안마다 옛 인용이 따라붙고 있었다.
사후 분류만 있었으면 사람이 그 초안을 읽고 지울 때까지 몰랐을 결함이다.

#### 검증 — 주입한 개입이 효과가 있었나 (`guidance_effect()`)

loop 에 측정·진단·개입은 있었지만 **검증이 없었다.** 프롬프트 규칙이 자동 주입되는데
그게 결함을 줄였는지는 아무도 재지 않았다. 효과 없는 규칙이 영원히 남는 것보다 나쁜
것은 **"조치했다" 는 착각** 이다 — 그 착각 때문에 진짜 레버(검색·지식)를 손대지 않는다.

단순 전후 비교는 쓸 수 없다. 활성 시점은 정의상 "그 원인이 많이 나온 시점" 이라,
개입에 아무 효과가 없어도 이후 발생률은 평균으로 되돌아가며 떨어진다. 그래서
**같은 클래스·같은 기간의 다른 원인을 대조군**으로 둔 이중차분을 쓴다:

    adjusted = (대상 원인 발생률 변화) − (다른 원인 발생률 변화)

대상만 줄면 크게 음수, 전체가 함께 줄면 0 근처. 표본이 임계 미만이면 **판정하지
않는다** — "효과 있음" 을 3건으로 선언하면 loop 가 노이즈를 근거로 개입을 유지한다.
효과 없음·악화로 판정되면 `escalate_lever` 제안이 개선 큐에 올라가 **다음 레버로**
넘긴다(generation → retrieval → knowledge). 판정만 하고 아무 일도 안 일어나면
검증을 붙인 의미가 없다.
...
.venv/bin/python tests/test_draft_defects.py      # 사전 탐지 — 오탐/등급/종류별 계약
.venv/bin/python tests/test_guidance_effect.py    # 개입 효과 — 평균회귀 함정 포함
```
...
src/
  ...
  draft_feedback.py     초안 거부·수정 원인 분류 축적 → 프롬프트/개선 큐 환류 + 개입 효과 검증
  draft_defects.py      초안 결함 사전 탐지(사람 검토 전) — 같은 원인 코드·레버
  guides.py             답변 지침 원천 (Confluence·FAQ·문서 → 섹션 + 검색)
  issue_chat.py         이슈 질의응답 — 근거 조립·집계·인용 검증
...
backend/server.py       FastAPI (/recommend, /issues/unresolved, /chat, /voc/reply/*, ...)
web/                    Vite + React + TS + Tailwind
  src/ReplyQueue.tsx    고객 답변 발송 대기 화면 (요청·정책 위반·미리보기·발송)
  src/ChatPage.tsx      이슈 질문(챗봇) — 근거 칩·스트리밍·환각 인용 경고
  src/issueLinks.tsx    이슈 키 → Jira 원본 링크 (마크다운 본문 포함)
```
